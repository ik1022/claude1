...
public:
    ...
    I2cStatus write(uint8_t slaveAddr, const uint8_t* data, size_t length) override {
        if (fail_next_write_) {
    ...
    I2cStatus read(uint8_t slaveAddr, [[maybe_unused]] uint8_t* data, [[maybe_unused]] size_t length) override {
        if (fail_next_read_) {
    ...
    I2cStatus writeRead([[maybe_unused]] uint8_t slaveAddr, [[maybe_unused]] uint8_t reg,
                        [[maybe_unused]] uint8_t* rxData, [[maybe_unused]] size_t rxLength) override {
        return I2cStatus::OK;
    }
